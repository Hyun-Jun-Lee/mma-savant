...
기존 AgentManager를 상속하여 2단계 추론 기능 추가
"""
import json
import asyncio
...
# SUPPORTED_CHARTS는 이제 JSON 파일에서 동적으로 로드됩니다
# src/llm/config/supported_charts.json 파일을 수정하여 차트 타입을 관리하세요
...
class AgentManagerV2(AgentManager):
    """
    Two-Phase Reasoning을 지원하는 확장된 에이전트 매니저
    Phase 1: 의도 분석 + 도구 선택/실행 (understand_and_collect)
    ...
    def __init__(self, mcp_server_path: Optional[str] = None):
        ...
        """
        super().__init__(mcp_server_path)
        
        # Two-Phase 전용 설정
        
        LOGGER.info("🎯 AgentManagerV2 initialized with Two-Phase Reasoning capability")
    ...
    async def _understand_and_collect(
        ...
        try:
            ...
            # Phase 1 전용 프롬프트 생성
            phase1_prompt = create_phase1_prompt_template()
            
            async with self.get_mcp_tools() as tools:
                LOGGER.info(f"🔧 Phase 1 loaded {len(tools)} MCP tools")
                
                # Phase 1 에이전트 생성 (도구 호출 중심)
                agent = create_tool_calling_agent(llm, tools, phase1_prompt)
                agent_executor = AgentExecutor(
                    ...
                    max_iterations=10,  # Phase 1은 여러 도구 호출 가능
                ...
                LOGGER.info("🚀 Phase 1: Starting LLM-driven tool execution...")
    ...
    def _parse_phase1_result(self, result: Dict[str, Any], processing_id: str) -> Dict[str, Any]:
        """Phase 1 결과 파싱"""
        try:
                ...
                if len(step) >= 2:
                    ...
                    tools_executed.append({
                        ...
                        "tool_input": getattr(action, 'tool_input', {}),
                            "observation": str(observation)[:500]  # 길이 제한
                    })
            ...
            return {
                ...
                "collected_data": self._extract_collected_data(intermediate_steps),
                "processing_instructions": self._extract_processing_instructions(output_text)
            }
            
        except Exception as e:
            LOGGER.error(format_exc())
            return {
                "error": f"Failed to parse Phase 1 result: {str(e)}",
                "phase": 1,
                "processing_id": processing_id
            }
    ...
    def _parse_phase2_result(self, response_text: str, processing_id: str, phase1_data: Dict[str, Any]) -> Dict[str, Any]:
        """Phase 2 결과 파싱 및 시각화 데이터 구조화 (직접 LLM 응답 처리)"""
        try:
            # 응답 텍스트 유효성 검사
            ...
            # 직접 JSON 파싱 시도 (프롬프트가 JSON만 반환하도록 지시)
            extracted_json = None
            try:
                # 먼저 직접 JSON 파싱 시도
    ...
    def _extract_key_insights_from_text(self, text: str) -> List[str]:
        ...
        if not isinstance(text, str):
            LOGGER.warning(f"Expected string input for insights extraction, got {type(text)}")
            return insights
    ...
    def _create_fallback_visualization(self, phase1_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "type": "text_summary",
            "data": {
                "content": f"데이터 수집이 완료되었습니다. {len(phase1_data.get('tools_executed', []))}개의 도구를 사용하여 정보를 조회했습니다.",
                "tools_used": [tool.get('tool', 'unknown') for tool in phase1_data.get('tools_executed', [])]
            }
        }
    ...
    def _extract_processing_instructions(self, output_text: str) -> Dict[str, Any]:
        """출력 텍스트에서 처리 지시사항 추출"""
        # 실제 구현에서는 더 정교한 파싱 로직 필요
        return {
            "format": "comprehensive_analysis",
            "include_visualizations": True,
            "korean_response": True
        }

    def _extract_visualizations(self, output_text: str) -> List[Dict[str, Any]]:
        """출력 텍스트에서 시각화 요소 추출"""
        # 실제 구현에서는 더 정교한 파싱 로직 필요
        return []

    def _extract_key_insights(self, output_text: str) -> List[str]:
        """출력 텍스트에서 핵심 인사이트 추출"""
        # 실제 구현에서는 더 정교한 파싱 로직 필요
        return []

    def _combine_results(self, phase1_result: Dict, phase2_result: Dict, processing_id: str) -> Dict[str, Any]:
        """Phase 1과 Phase 2 결과 통합"""
        return {
            "processing_id": processing_id,
            "user_response": phase2_result.get("final_response", ""),
            "phase1_summary": {
                "tools_used": len(phase1_result.get("tools_executed", [])),
                "data_collected": bool(phase1_result.get("collected_data")),
            },
            "phase2_summary": {
                "visualizations_prepared": len(phase2_result.get("visualizations", [])),
                "insights_generated": len(phase2_result.get("key_insights", [])),
            },
            "detailed_results": {
    ...
    def _get_cached_result(self, user_query: str) -> Optional[Dict[str, Any]]:
        """캐시된 결과 조회"""
        # 간단한 캐싱 구현 - 실제로는 더 정교한 캐싱 필요
        cache_key = hash(user_query)
        return None  # 현재는 캐싱 비활성화

    def _cache_result(self, user_query: str, result: Dict[str, Any]):
        """결과 캐싱"""
        # 간단한 캐싱 구현 - 실제로는 더 정교한 캐싱 필요
        pass
    ...
    def get_processing_stats(self) -> Dict[str, Any]:
        ...
        if stats["total_queries"] > 0:
            stats["phase1_success_rate"] = stats["phase1_success"] / stats["total_queries"]
            stats["phase2_success_rate"] = stats["phase2_success"] / stats["total_queries"]
            stats["overall_success_rate"] = min(stats["phase1_success_rate"], stats["phase2_success_rate"])
    ...
    async def health_check_v2(self) -> Dict[str, Any]:
        ...
        v2_health = {
            "two_phase_system": "healthy",
            "processing_stats": self.get_processing_stats(),
            "cache_status": {
                "phase1_cache_size": len(self._phase1_cache),
                "phase2_cache_size": len(self._phase2_cache)
            }
        }
        
        base_health.update(v2_health)
        return base_health


# 편의 함수들
async def process_query_with_two_phase(
    user_query: str,
    llm: Any,
    callback_handler: Any,
    chat_history: Optional[List[BaseMessage]] = None
    ...
    return await manager.process_two_step(
        user_query=user_query,
        llm=llm,
        callback_handler=callback_handler,
        chat_history=chat_history
...
if __name__ == "__main__":
    ...
    async def test_agent_manager_v2():
        ...
        # 상태 확인
        print("\n🏥 Health Check:")
        health = await manager.health_check_v2()
    ...
    if asyncio.get_event_loop().is_running():
        ...
        asyncio.run(test_agent_manager_v2())