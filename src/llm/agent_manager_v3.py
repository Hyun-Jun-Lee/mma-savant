...
class AgentManagerV2(AgentManager):
    """
    Two-Phase Reasoning을 지원하는 확장된 에이전트 매니저 (ReAct + OpenRouter)
    ...
    def _create_react_compatible_tools(self):
        ...
        MCP 도구의 JSON 스키마 문제를 해결하기 위해 문자열 입력을 받는 도구 사용
        """
        import json
        ...
        def sync_execute_sql_query(query: str) -> str:
            ...
            try:
                ...
                with get_readonly_db_context() as session:
                    ...
                    # 결과를 JSON 형식으로 반환 (test_openrouter_2.py 방식)
    ...
    async def _understand_and_collect(
        ...
        try:
            ...
            # ReAct 에이전트용 일반 LangChain 도구 생성 (MCP 호환성 문제 해결)
            ...
            agent_executor = AgentExecutor(
                ...
                # early_stopping_method 제거 (기본값 사용)
            )
