...
class MatchService:
    ...
    async def get_all_matches(self, event_id: int):
    ...
    async def get_match_statistics(self, match_id: int):
        return await self.match_repo.get_match_statistics(match_id)

    async def get_punch_accuracy_by_result(self, fighter_id: int, result: str) -> float:
        ...
        return landed_punches / total_punches if total_punches > 0 else 0.0