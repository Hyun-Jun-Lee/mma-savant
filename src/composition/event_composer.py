...
from composition.dto import (
    ...
    EventSummaryDTO, EventStatsDTO
)

async def get_event_summary(session: AsyncSession, event_id: int) -> EventSummaryDTO:
    """
    ...
    try:
        ...
        if not event:
            raise CompositionNotFoundError("Event", event_id, "get_event_summary")
        ...
        raise CompositionQueryError("get_event_summary", {"event_id": event_id}, str(e))


async def get_event_with_all_matches(session: AsyncSession, event_name: str) -> EventWithAllMatchesDTO:
    """
    특정 이벤트에 속한 모든 경기와 승패결과를 조회합니다.
    """
    # 입력 검증
    if not event_name or not event_name.strip():
        raise CompositionValidationError("event_name", event_name, "Event name cannot be empty")
    
    try:
        # 이벤트 이름으로 이벤트 조회
        event = await event_repo.get_event_by_name(session, event_name)
        if not event:
            raise CompositionNotFoundError("Event", event_name, "get_event_with_all_matches")
        
        # 이벤트의 모든 매치와 요약 정보 조회
        event_summary = await get_event_with_matches_summary(session, event.id)
        
        if not event_summary:
            raise CompositionNotFoundError("Event summary", event.id, "event with matches")
        ...
        raise CompositionQueryError("get_event_with_all_matches", {"event_name": event_name}, str(e))
...
async def get_recent_events_with_main_match(session: AsyncSession, limit: int = 10) -> List[EventWithMainMatchDTO]:
    ...
    try:
        ...
        raise CompositionQueryError("get_recent_events_with_main_match", {"limit": limit}, str(e))
...
async def get_upcoming_events_with_featured_matches(session: AsyncSession, limit: int = 5) -> List[UpcomingEventWithFeaturedMatchesDTO]:
    ...
    try:
        ...
        raise CompositionQueryError("get_upcoming_events_with_featured_matches", {"limit": limit}, str(e))


async def compare_events_by_performance(session: AsyncSession, event_id1: int, event_id2: int) -> EventComparisonDTO:
    """
    두 이벤트의 성과를 비교 분석합니다.
    """
    # 입력 검증
    if not isinstance(event_id1, int) or event_id1 <= 0:
        raise CompositionValidationError("event_id1", event_id1, "event_id1 must be a positive integer")
    if not isinstance(event_id2, int) or event_id2 <= 0:
        raise CompositionValidationError("event_id2", event_id2, "event_id2 must be a positive integer")
    ...
    try:
        ...
        if not event1_summary:
            raise CompositionNotFoundError("Event", event_id1, "compare_events_by_performance")
        if not event2_summary:
            raise CompositionNotFoundError("Event", event_id2, "compare_events_by_performance")
        ...
        raise CompositionQueryError("compare_events_by_performance", {"event_id1": event_id1, "event_id2": event_id2}, str(e))
...
async def get_event_rankings_impact(session: AsyncSession, event_id: int) -> EventRankingImpactDTO:
    ...
    try:
        ...
        raise CompositionQueryError("get_event_rankings_impact", {"event_id": event_id}, str(e))