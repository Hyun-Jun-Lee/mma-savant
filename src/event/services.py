...
async def get_event_timeline(session: AsyncSession, period: str = "month") -> Dict[str, Any]:
    """
...
async def get_event_summary(session: AsyncSession, event_id: int) -> Optional[Dict[str, Any]]:
    """
...
async def get_location_statistics(session: AsyncSession) -> Dict[str, Any]:
    """
...
async def get_next_and_last_events(session: AsyncSession) -> Dict[str, Any]:
    """
    ...
    if next_event and next_event.event_date:
        days_until = (next_event.event_date - date.today()).days
        result["days_until_next"] = days_until
    
    # 마지막 이벤트로부터 경과 일수 계산  
    if last_event and last_event.event_date:
        days_since = (date.today() - last_event.event_date).days
        result["days_since_last"] = days_since
    
    return result


async def get_event_trends(session: AsyncSession, period: str = "yearly") -> Dict[str, Any]:
    """
    이벤트 개최 트렌드를 분석합니다.
    """
    current_year = date.today().year
