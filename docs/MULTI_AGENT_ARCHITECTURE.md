...
| Critic 재시도 범위 | `active_agents` 전체 재시도 (`Send()` 동적 디스패치) | 구현 단순, 실패 원인 에이전트 식별 불필요. Phase 1에 적합 |
| 에러 핸들링 | 기존 `sql_agent` 패턴 적용 (try/except + 타임아웃 + 에러 AgentResult) | 검증된 패턴 재사용, 모든 에이전트 동일 구조 |
| State 초기값 | `.get()` 안전 접근 방식 | TypedDict는 기본값 미지원, 노드에서 `.get(key, default)`로 처리 |
...
main_graph.add_conditional_edges(
    ...
    ["direct_response", "mma_analysis", "fighter_comparison", "enrichment"]  # path_map: 가능한 타겟 노드 목록
)
```

**Fan-in (병렬 결과 합류)**:
`Send()`로 병렬 활성화된 에이전트들은 `agent_results: Annotated[list[AgentResult], reduce_agent_results]` 커스텀 reducer를 통해
결과가 자동 합산된다. 모든 병렬 에이전트가 완료되면 `add_edge`로 선언된 다음 노드(critic)로 진행한다.
...
**재시도 흐름 (LangGraph 구현)**:
```
에이전트 → critic ─(통과)─→ Send(text_response + visualization)
                  └─(실패, retry < 3)─→ Send(active_agents) (피드백 포함, agent_results 초기화)
                  └─(실패, retry >= 3)─→ END (에러 응답)
```

`critic_route`는 string, `END`, `list[Send]`를 혼합 반환할 수 있으므로
**별도의 `response_fanout` 노드 없이** critic에서 직접 `Send()`로 분기한다.
...
def critic_route(state: MainState):
    ...
    Returns:
        list[Send] — 통과 시: 텍스트(항상) + 시각화(조건부) 병렬 Send
        list[Send] — 재시도 시: active_agents 전체 재실행 Send
        END — 3회 소진 시: 그래프 종료
    """
    if state.get("critic_passed", False):
        # 통과 → 텍스트(항상) + 시각화(조건부) 병렬 실행
        sends = [Send("text_response", state)]
        if any(r["needs_visualization"] for r in state.get("agent_results", [])):
    ...
    return [Send(agent, state) for agent in state.get("active_agents", [])]
...
**재시도 시 `active_agents` 전체 재실행 근거**:
- 복합 질문(`complex`)에서 mma_analysis + fighter_comparison이 병렬 실행된 후 Critic이 실패하면,
  어떤 에이전트 결과가 문제인지 Critic이 식별하기 어렵다.
- 전체 재실행이 가장 단순하고, 재시도 횟수가 최대 3회이므로 비용 부담이 제한적이다.
- Critic 재시도 시 `agent_results: []`를 반환하여 이전 실패 결과를 초기화한다.
...
async def critic_node(state: MainState, llm) -> dict:
    ...
    return {
        "critic_passed": False,
        "critic_feedback": f"검증 실패: {feedback_message}",
        "retry_count": retry_count,
        "agent_results": [],  # reducer가 초기화 → 재실행 시 깨끗한 상태
    }
```
...
**병렬 실행 전략 (critic_route 통합)**:

별도의 `response_fanout` 노드 없이, `critic_route` 함수가 직접 `Send()`를 반환하여
텍스트와 시각화를 동시에 활성화한다 (Critic Agent 섹션의 `critic_route` 참조).

```
critic ─(통과)─→ Send("text_response") ────→ text_response ──→ END
               └─Send("visualization") (조건부)→ visualization ──→ END
```

- `critic_route`가 `list[Send]`를 직접 반환하므로 중간 노드 불필요
...
# ── 순차 에지 ──
main_graph.add_edge(START, "conversation_manager")
main_graph.add_edge("conversation_manager", "supervisor")

# ── fan-in 에지: 분석 에이전트들 → critic ──
main_graph.add_edge("mma_analysis", "critic")
main_graph.add_edge("fighter_comparison", "critic")

# ── 터미널 에지 ──
main_graph.add_edge("direct_response", END)
main_graph.add_edge("text_response", END)
main_graph.add_edge("visualization", END)

# ── 동적 라우팅 (Send() 기반, path_map으로 가능한 타겟 노드 명시) ──
main_graph.add_conditional_edges(
    "supervisor", supervisor_dispatch,
    ["direct_response", "mma_analysis", "fighter_comparison", "enrichment"]
)
main_graph.add_conditional_edges(
    "critic", critic_route,
    ["text_response", "visualization", "mma_analysis", "fighter_comparison", END]
...
**State 필드 접근 컨벤션 — `.get()` 안전 접근**:
TypedDict는 기본값을 지원하지 않으므로, 그래프 초기 호출 시 설정되지 않은 필드에 접근하면 `KeyError`가 발생한다.
모든 노드에서 state 필드를 읽을 때는 `.get(key, default)` 패턴을 사용한다.
...
모든 에이전트 노드는 기존 `sql_agent.py`의 에러 핸들링 패턴을 따른다.
`GraphRecursionError`, `TimeoutError`, 일반 `Exception`을 각각 처리하여
에이전트 실패가 전체 그래프를 중단시키지 않도록 한다.
...
async def agent_node(state: MainState, llm) -> dict:
    try:
        result = await asyncio.wait_for(
            _run_agent(state, llm),
            timeout=AGENT_TIMEOUT_SECONDS,
        )
        return result

    except GraphRecursionError:
        LOGGER.error(f"❌ Agent recursion limit reached ({AGENT_RECURSION_LIMIT})")
        return {
            "agent_results": [_error_agent_result(
                agent_name="...",
                error="쿼리 생성 과정이 너무 복잡합니다. 질문을 더 구체적으로 바꿔주세요.",
            )],
        }

    except asyncio.TimeoutError:
        LOGGER.error(f"❌ Agent timed out after {AGENT_TIMEOUT_SECONDS}s")
        return {
            "agent_results": [_error_agent_result(
                agent_name="...",
                error=f"처리 시간이 {AGENT_TIMEOUT_SECONDS}초를 초과했습니다.",
            )],
        }
...
| 에이전트 | 타임아웃 | 에러 시 동작 |
|---------|---------|------------|
| Conversation Manager | 10초 | 원본 질문으로 폴백 (`resolved_query = 원본 메시지`) |
| Supervisor | 10초 | 기본 라우팅 폴백 (`route = "mma_analysis"`) |
| MMA 분석 | 30초 | 에러 AgentResult 반환 → Critic이 실패 판정 |
| Fighter 비교 | 30초 | 에러 AgentResult 반환 → Critic이 실패 판정 |
| Critic | 15초 | 규칙 기반만 적용, LLM 실패 시 규칙 기반 결과로 통과/실패 판정 |
| 텍스트 응답 | 15초 | reasoning 폴백 (있으면 그대로 사용) |
| 시각화 | 15초 | 시각화 생략 (텍스트 응답은 이미 별도 완성) |

**Conversation Manager / Supervisor 에러 폴백**:
이 두 노드는 그래프 초입부에 있어 실패 시 전체 흐름이 중단된다.
따라서 에러 시 AgentResult가 아닌 **안전한 기본값으로 폴백**한다.

```python
async def conversation_manager_node(state: MainState, llm) -> dict:
    try:
        resolved_text = await _resolve_query(state, llm)
        return {
        ...
        # 폴백: 원본 메시지를 그대로 사용
        original = state.get("messages", [])[-1].content if state.get("messages") else ""
        return {
...
이후 모든 노드가 의존하는 기반을 구축하는 단계.

- `MainState` 정의 (`AgentResult`, `reduce_agent_results` reducer 포함)
- `_error_agent_result` 헬퍼 함수 (에러 시 AgentResult 생성, 모든 노드가 공유)
- `MAIN_MODEL` / `SUB_MODEL` 환경변수를 `Config` 클래스에 추가
- `MAIN_MODEL` / `SUB_MODEL` 환경변수 파싱 (`_parse_model_spec`)
- `get_main_model()` / `get_sub_model()` 팩토리 함수
- `SupervisorRouting` 등 신규 Pydantic 스키마

**산출물**: `state.py` (재작성), `config.py` (확장), `model_factory.py` (확장), `schemas.py` (확장)

### Step 2: 개별 노드 + 프롬프트 구현

각 노드를 독립적으로 구현. 프롬프트도 함께 작성하여 노드가 단독 실행 가능한 상태로 만든다.
모든 노드는 에러 핸들링 패턴(try/except + 타임아웃 + 에러 AgentResult/폴백)을 적용한다.

- **Conversation Manager 노드 + 프롬프트**: 히스토리 압축 + 대명사 해소 + `resolved_query` 반환 + 에러 시 원본 질문 폴백
- **Supervisor 노드 + 프롬프트**: `resolved_query` 기반 라우팅 + `SupervisorRouting` structured output + 에러 시 기본 라우팅 폴백
- **MMA 분석 노드**: 기존 `sql_agent_node` 리팩토링 → `AgentResult` 반환 형태로 변환 + 에러 시 에러 AgentResult
- **Fighter 비교 노드 + 프롬프트**: MMA 분석과 동일 구조, 비교 전용 프롬프트 + SQL 패턴 + 에러 시 에러 AgentResult
- **Critic 노드 + 프롬프트**: 규칙 기반 검증(Phase A) + LLM 정합성 검증(Phase B) + `retry_count` 관리 + `agent_results` 초기화
- **텍스트 응답 노드**: reasoning 재사용 로직 + 복수 에이전트 결과 통합 + 에러 시 reasoning 폴백
- **시각화 노드**: 기존 `visualize_node` → `agent_results` 입력으로 변환 + 에러 시 시각화 생략
- **direct_response 노드**: 기존 로직 유지

**산출물**: `nodes/` 디렉토리 내 각 노드 파일, `prompts.py` (확장)
...
- 순차 에지: `START → conversation_manager → supervisor`
- fan-in 에지: `mma_analysis → critic`, `fighter_comparison → critic`
- 터미널 에지: `direct_response → END`, `text_response → END`, `visualization → END`
- `supervisor_dispatch` 라우팅 함수: `Send()` 기반 동적 fan-out (path_map 포함)
- `critic_route` 라우팅 함수: 직접 `Send()` 반환 (통과 → text+viz, 재시도 → active_agents, 에러 → END)
- general fast path (Supervisor → direct_response → END)

**산출물**: `graph_builder.py` (재작성 — 라우팅 함수 `supervisor_dispatch`, `critic_route` 포함)
...
**참고**: `config.py`의 Config 클래스 수정은 Step 1에서 완료되어 있다.
...
Step 2에서 작성한 프롬프트를 실제 서비스 환경에서 검증하고, 전체 흐름을 E2E로 테스트한다.

- 프롬프트 품질 검증 및 튜닝 (실제 LLM 응답 기반)
- E2E 시나리오 테스트:
  ...
  - Critic 3회 소진 → 에러 응답 반환
  - 에이전트 타임아웃 → 에러 AgentResult → Critic 실패 판정
  - Conversation Manager 실패 → 원본 질문 폴백으로 정상 진행
